...
async def executor_node(state: GraphState) -> dict:
    ...
    result = {
        "status": "done",
        "result": f"任务 {next_task.title} 执行完成",
        "specialist_id": specialist_id,
        "finished_at": datetime.now(),
    }

    # 如果 subagent 返回了实际结果，使用它
    if call_result.get("success") and call_result.get("result"):
        result["result"] = call_result["result"]
